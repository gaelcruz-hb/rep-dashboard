...
export function useChannelsData({ period, startDate, endDate } = {}) {
  const [data, setData]       = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError]     = useState(null);

  const filterKey = `${period ?? ''}|${startDate ?? ''}|${endDate ?? ''}`;
  const cacheKey  = `channels-data:${filterKey}`;
  ...
  useEffect(() => {
    ...
    const url = `${API_URL}/api/channels-data${qs ? '?' + qs : ''}`;
    ...
    const fetchData = async (bypassCache = false) => {
      ...
      try {
        ...
        console.error('[Channels] Failed to fetch channels data:', err);
